...
public class AdminProductService extends AdminServiceSupport {
    ...
    public AdminProductService(ProductRepository products, CategoryRepository categories) { this.products = products; this.categories = categories; }
    ...
    private void copy(Product p, ProductRequest r) { p.setName(r.name()); if (r.category() != null && !r.category().isBlank()) { Category category = categories.findByNameIgnoreCase(r.category().trim()).orElseGet(() -> { Category created = new Category(); created.setName(r.category().trim()); return categories.save(created); }); p.setCategoryEntity(category); } p.setDescription(r.description()); p.setPrice(r.price()); p.setStock(r.stock()); p.setImageUrl(r.imageUrl()); }
}
